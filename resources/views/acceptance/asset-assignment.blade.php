...
<head>
    ...
    <title>Accept Asset Receipt</title>
    <style>
...
<main>
    <div class="card">
        ...
        <h1>Asset Receipt Acceptance</h1>
        ...
        @else
            <div class="notice">
                Confirm that you received this asset and accept responsibility according to your company policy.
            </div>
        @endif

        <div class="grid">
            <div class="label">Issued to</div><div>{{ $assignedTo }}</div>
            <div class="label">Asset tag</div><div>{{ $asset?->asset_tag ?? '-' }}</div>
            <div class="label">Model</div><div>{{ $asset?->assetModel?->name ?? '-' }}</div>
            <div class="label">Serial</div><div>{{ $asset?->serial ?? '-' }}</div>
            <div class="label">Category</div><div>{{ $asset?->category?->name ?? '-' }}</div>
        ...
        @if (! $assignment->accepted_at)
            <form method="post" action="{{ route('assetflow.acceptance.asset.accept', [$assignment, $token]) }}">
                @csrf
                <label>
